...
function buildRecommendation(filter, reason, confidence, warnings) {
  return {
...
export function estimateNDFromWeather(input = {}) {
  ...
  if (hasPrecipitation || isRain) {
    const filter = cloudCover !== null && cloudCover < 60 && visibility !== null && visibility >= 5000 ? "ND4" : "ND0";
  ...
  if (hasSunTimes) {
    ...
    if (isBetween(time, sunrise, morningGoldenEnd) || isBetween(time, eveningGoldenStart, sunset)) {
      const filter = cloudCover !== null && cloudCover >= 60 ? "ND4" : "ND8";
