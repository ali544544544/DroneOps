...
const I18n = {
  ...
  init(data) {
    ...
    this.locale = this.lang === 'de' ? 'de-DE' : 'en-US';
  },
  t(key) {
  ...
  setLanguage(lang) {
    this.lang = lang;
    this.locale = lang === 'de' ? 'de-DE' : 'en-US';
...
const LocationManager = {
  ...
  saveAll(list) { Storage.set(Keys.locations, list); },
  getById(id) { return this.getAll().find(l => l.id === id); },
  add(location) {
    const list = this.getAll();
    const next = { id: Util.uuid(), ...location, notes: '', logbook: [] };
    list.push(next);
    this.saveAll(list);
    return next;
  },
  update(id, patch) {
    const list = this.getAll().map(l => l.id === id ? { ...l, ...patch } : l);
    this.saveAll(list);
  },
  remove(id) {
  ...
  addLog(locationId, entry) {
    const loc = this.getById(locationId);
    if (!loc) return;
    if (!loc.logbook) loc.logbook = [];
    loc.logbook.unshift({ id: Util.uuid(), date: new Date().toISOString(), ...entry });
    this.update(locationId, { logbook: loc.logbook });
  },
  removeLog(locationId, logId) {
    const loc = this.getById(locationId);
    if (!loc) return;
    loc.logbook = loc.logbook.filter(e => e.id !== logId);
    this.update(locationId, { logbook: loc.logbook });
  }
};
...
const ProfileManager = {
  init(profiles) {
    if (!Storage.get(Keys.profiles)) {
      Storage.set(Keys.profiles, profiles || FALLBACK_PROFILES);
    }
  },
  ...
  label(id) { return this.getById(id)?.label || 'Unbekannt'; },
  getActive() { 
...
const ChecklistManager = {
  init() {
    if (!Storage.get(Keys.checklist)) {
      Storage.set(Keys.checklist, [
        { id: '1', name: 'Akkus geladen', count: 4, category: 'akkus', checked: false },
        { id: '2', name: 'SD-Karte im Copter', count: 1, category: 'elektronik', checked: false },
        { id: '3', name: 'Propeller fest', count: 1, category: 'zubehoer', checked: false }
      ]);
    }
  },
  ...
  reset() {
    const items = this.getAll().map(i => ({ ...i, checked: false }));
    this.saveAll(items);
  },
  categories() {
    const cats = this.getAll().map(i => i.category);
    return [...new Set(cats)];
  }
};
...
const WeatherManager = {
  async getForecast(lat, lon) {
    ...
    try {
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,showers,snowfall,weather_code,cloud_cover,pressure_msl,surface_pressure,wind_speed_10m,wind_direction_10m,wind_gusts_10m,visibility&hourly=temperature_2m,precipitation_probability,weather_code,wind_speed_10m,wind_gusts_10m&timezone=auto&forecast_days=3`;
...
const SunManager = {
  async getTimes(lat, lon) {
    const cacheKey = `${lat},${lon}_${new Date().toISOString().slice(0, 10)}`;
    const cache = Storage.get(Keys.sunCache, {});
    ...
    try {
      ...
      Storage.set(Keys.sunCache, cache);
      return results;
    } catch (e) { console.error(e); return null; }
  },
  getGoldenHour(times) {
    if (!times) return null;
    const morningStart = new Date(new Date(times.sunrise).getTime() - 30 * 60 * 1000);
    const morningEnd = new Date(new Date(times.sunrise).getTime() + 60 * 60 * 1000);
    const eveningStart = new Date(new Date(times.sunset).getTime() - 60 * 60 * 1000);
    const eveningEnd = new Date(new Date(times.sunset).getTime() + 30 * 60 * 1000);
    const now = new Date();
    let whichPhase = null;
